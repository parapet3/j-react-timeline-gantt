...
export default class Header extends PureComponent {
  ...
  renderHeaderRows = (top, middle, bottom) => {
    ...
    let start = this.props.currentday; //integer, returns 0
    ...
    */
    for (let i = start - BUFFER_DAYS; i < end + BUFFER_DAYS; i++) {
      ...
      let tempBottom;
      if (this.getFormat(bottom) == "WC") {
        let temp = getMonday(currentDate);
        tempBottom = temp.format("DD");
      } else {
        tempBottom = currentDate.format(this.getFormat(bottom));
      }

      if (currentBottom != tempBottom) {
  ...
  renderHeader = () => {
    switch (this.props.mode) {
      ...
      case VIEW_MODE_YEAR:
        ...
        //Julian was here
        return this.renderHeaderRows("year", "month", "dayyeear");
    }
  };
...
function getMonday(d) {
  //d = new Date(d);
  let day = d.day();
  let diff = day == 0 ? -6 : 1; // adjust when day is sunday to return previous week.
  let result = d.day(diff);
  //console.log("d: " + d.format("ddd DD MMMM YYYY") + ", Adjusted Date: " + result.format("ddd DD MMMM YYYY"));
  return result;
}
